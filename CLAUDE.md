...
- **Stop the build-fix loop**: After 2–3 attempts at the same class of failure (compile error, test failure, build failure, deploy failure), STOP and produce a structured summary via the `/diagnose-loop` skill before attempting another fix. The summary must include the exact failing command, the error class (e.g., "H2 doesn't support this DDL", "scalafix keeps stripping the import", "CRLF artifact"), what was tried in each attempt, the working hypothesis for root cause, the next move and why, and the alternative if the hypothesis is wrong. Ask the user before continuing. "Same class" includes same exit code with similar error text, same test failing for similar reasons, same build phase failing on similar inputs — a new variant of the same syntactic tweak counts as the same class. Only switching to a meaningfully different approach (different backend, different tool, different abstraction) resets the count.
- **Verify CI with fresh data**: `gh pr checks <num>` and `gh pr view --json statusCheckRollup` return the LATEST check-runs known to GitHub, which can include stale results from the previous commit during the 5–30 second window between `git push` and GitHub Actions enqueuing the new run. After pushing, never report CI status from the first response. Anchor on the commit SHA: capture `SHA=$(git rev-parse HEAD)` BEFORE pushing, then poll `gh api repos/<owner>/<repo>/commits/$SHA/check-runs` every 5–10 seconds until (a) at least one check run for that SHA appears, then (b) every check run has a terminal `conclusion` (success/failure/cancelled/timed_out). Only then report. `gh pr checks --watch` alone is insufficient — if the previous run is already terminal-green it exits immediately before the new run is enqueued.
- **Streaming code must not buffer end-to-end**: FS2 / streaming code must process elements as they arrive, not collect everything then batch-process. Antipatterns: `.compile.toList` followed by mass processing; `Stream.eval(loadAll)` for eager full collection wrapped in Stream; `.fold` building a buffer that's emitted only at completion; `groupWithin(_, _)` with no upper bound on group size or time. Use `parEvalMap(parallelism)` for parallel processing, `evalMap` for sequential, bounded `groupWithin` for batching. If you genuinely need the entire collection materialized before processing, use `Vector` + parallel collection + batch APIs and be explicit about it — don't dress it up as a stream. Streaming buys bounded memory and incremental output; if you defeat both, you've added complexity for nothing.
- **Confirm working location at task start**: When multiple worktrees or repos are open, the first `Edit`/`Write`/git operation of a task must come AFTER stating `pwd`, `git branch --show-current`, and `git remote -v` for the working directory. Confirm with the user if any of those don't match what they described. Catches a class of mistakes where the agent dives in and writes files to the wrong worktree, commits to the wrong branch, or scaffolds into the wrong repo entirely. Pairs with the blocking branch-state PreToolUse hook in `.claude/settings.json` — that hook catches the wrong-branch case at push time; this rule catches the wrong-location case BEFORE any changes are made.
...
- **Coverage gates are architecture feedback**: All newly created or changed code must have test coverage above 90% (enforced by Codecov patch coverage on PRs). Run `sbt coverage test coverageReport` locally to verify before pushing. **When a file is hard to reach 90% on, the code shape (constructor wires too many concerns, methods do multiple things, dependencies are hard-coded) is usually the problem — not "needs more tests".** The FIRST move is the testability refactoring pattern below; only then add tests against the refactored shape. Adding tests on top of bad shape produces brittle mock-heavy specs. Never add files to the `ignore` list in `codecov.yml` to work around missing coverage.
