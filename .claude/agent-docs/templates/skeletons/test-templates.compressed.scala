...
- Negative testing: analyze each function line for failure inputs using equivalence classes, create test per failure scenario
- If production code lacks error handling path → add one first
...
- Integration (WireMock): failure simulation only (timeouts, errors, malformed responses) with validated response fixtures
- Integration (dev GCP): contract/connection validation against real dev project, namespaced per developer/CI run
...
**When To Use:**
- Testing API client behavior with deterministic canned responses
- No network calls, fully isolated from external services
...
**When To Use:**
- Testing every failure path in a function
- Ensuring proper error handling and exception uniqueness
- Building confidence in edge case coverage
...
class BillTypesNegativeSpec extends AnyFlatSpec with Matchers {
  ...
  //   - Input: null → NullPointerException (WartRemover prevents null)
  ...
  //   2. Valid code wrong case → Left (case sensitivity)
...
class RetryWrapperNegativeSpec extends AnyFlatSpec with Matchers {
  ...
  //   2. Fails, succeeds within retries → should succeed
  //   3. Transient error exhausts all retries → TransientFailure
  //   4. Systemic error on first try → SystemicFailure immediately
  //   5. Operation times out → classified by ErrorClassifier
  ...
  it should "respect timeout per operation" in {
    // Operation exceeding config.timeout
    // Expect timeout → TransientFailure or SystemicFailure per classifier
    pending
  }

  it should "cap backoff at maxBackoff" in {
    // After many retries, backoff <= config.maxBackoff
    pending
  }
}
```
...
**When To Use:**
- Testing database persistence and queries
- Full schema in play (Liquibase migrations)
- Local development and CI (requires Docker)

**Prerequisites:**
- build.sbt: `.dependsOn(dbMigrations % "test->test")`
- Docker running locally
- DockerPostgresSpec handles: starting AlloyDB Omni (google/alloydbomni:16.8.0) container with random port, applying Liquibase migrations, exposing jdbcUrl and getConnection, removing container in afterAll

**Important:** Test data must have unique values for ALL unique constraints on target table, not just primary key. Check migration SQL for constraint definitions. Expose parameters for every constrained column in test data helpers.
...
class MyRepositoryIntegrationSpec extends AnyFlatSpec with Matchers {
  ...
  //       url = jdbcUrl,          // from DockerPostgresSpec
  //       user = "test",
  //       password = "test",
  //       logHandler = None,
  //     )
  ...
  //     naturalKey1: Int = 1,        // UNIQUE constraint
  //     naturalKey2: String = "type", // UNIQUE constraint
  //     title: String = "Default",
  //   ): MyEntityDO = MyEntityDO(id, naturalKey1, naturalKey2, title, ...)
  //
  //   // Verification helper: read back from DB
...
**When To Use:**
- Simulating timeouts, error responses, malformed responses, rate limits from external services
- Validating retry logic and error classification
- Response fixtures must be validated against known real responses
...
**When To Use:**
- Validating contracts (request/response shapes) against real dev environment
- Verifying connectivity and persistence to real AlloyDB dev instance
- NOT for failure simulation (use WireMock for that)

**Namespacing:** Each test run uses unique schema prefix (underscores for valid PostgreSQL names) per developer/CI run to avoid collisions.
