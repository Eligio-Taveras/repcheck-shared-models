...
**Purpose:** Cloud Run Job entry point pattern. Extends IOApp, loads snapshots from GCS, processes items via FS2 streaming with fail-and-continue, writes PipelineRunSummary, and updates run status in DB.
...
Base pattern for all pipeline apps. Copy and fill in:
...
```scala
/** Base pattern for all pipeline apps. */
trait PipelineApp {
