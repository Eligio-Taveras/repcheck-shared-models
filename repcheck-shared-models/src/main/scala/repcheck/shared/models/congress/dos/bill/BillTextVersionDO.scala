 ...
 *
 * As of db-migrations 026 this DO holds only version metadata: which version exists for the bill, where its canonical
 * URL points, when it was fetched. The actual text and per-chunk embeddings live in [[RawBillTextDO]] rows, joined by
 * `version_id`. A version-row without any `RawBillTextDO` children means the text hasn't been fetched + embedded yet.
 */
